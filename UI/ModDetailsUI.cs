...
namespace Rooster.UI
{
    public static class ModDetailsUI
    {
        ...
        public static void ShowDetails(ThunderstorePackage pkg)
        {
            ...
            // Clean previous custom UI or messages
            UIHelpers.CleanContainer(modal.simpleMessageContainer.gameObject, modal.simpleMessageText.gameObject);
            ...
            if (!string.IsNullOrEmpty(pkg.WebsiteUrl))
            {
                ...
                if (viewBtn != null)
                {
                    ...
                    viewBtn.OnClick.AddListener((cursor) =>
                    {
                        ...
                        if (SecurityService.IsValidWebUrl(targetUrl))
                        {
                            Application.OpenURL(targetUrl);
                        }
                        else
                        {
                            RoosterPlugin.LogWarning($"[Security] Blocked attempt to open invalid/local URL: {targetUrl}");
                        }
                    });
                                ...
                                installAllBtn.OnClick.AddListener((c) =>
                                {
                                    ...
                                    installNext = () =>
                                    {
                                        if (depIdx < missingDeps.Count)
                                        {
                                            ...
                                            InstallOne(d, installNext, (err) =>
                                            {
                                                modal.ShowSimpleMessage("Dependency Install Failed", $"Failed to install {d.Name}: {err}", () => ShowDetails(pkg));
                                            });
                                        }
                                        else
                                        {
                                            // Finally install the main mod
                                            InstallOne(pkg, () => ShowDetails(pkg), (err) =>
                                            {
                                                modal.ShowSimpleMessage("Install Failed", err, () => ShowDetails(pkg));
                                            });
                                        }
                                    };
                                    installNext();
        ...
        private static void InstallOne(ThunderstorePackage pkg, Action onSuccess, Action<string> onError)
        {
            ...
            // Cleanup details view before showing progress
            CleanupCustomUI();

            var modal = Tablet.clickEventReceiver.modalOverlay;
            modal.ShowSimpleMessage("Installing...", $"Downloading and installing {pkg.Name}...", null);
                ...
                if (success)
                {
                    UpdateInstaller.InstallPackage(tempPath, pkg, (installSuccess, installErr) =>
                    {
                        if (installSuccess)
                        {
                            ...
                            UIHelpers.ShowRestartPrompt(modal, "Install Successful", 
                                $"{pkg.Name} has been installed successfully!\nA restart is required to load the new mod.", 
                                onSuccess);
                        }
                        else
                        {
