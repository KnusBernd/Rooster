...
namespace Rooster.Services
{
    ...
    public static class UpdateInstaller
    {
        ...
        private static string ExtractAndFindRoot(string zipPath, string extractPath)
        {
            ...
            // Check for BepInEx folder even without manifest (direct ZIP contains BepInEx/...)
        ...
        private static void ClearBepInExCache()
        {
            try
            {
                ...
                // Rooster Icon cache maintenance
                string iconCacheDir = Path.Combine(cacheDir, "Rooster", "Icons");
                if (Directory.Exists(iconCacheDir))
                {
                    var files = new DirectoryInfo(iconCacheDir).GetFiles("*.png");
                    // If more than 500 icons, clear the oldest ones or just clear all if it's too much
                    if (files.Length > 500)
                    {
                        foreach (var f in files) f.Delete();
                        RoosterPlugin.LogInfo("[Cleanup] Icon cache cleared (reached item limit).");
                    }
                }
            }
            catch (Exception ex) { RoosterPlugin.LogWarning($"Failed to clear cache: {ex.Message}"); }
        }
